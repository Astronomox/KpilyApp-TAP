...
import { finishPasswordReset, getSession, updatePassword } from '@/lib/kpily'
...
function ChangePassword() {
  // Reset link: /auth/reset-password/{code} or ?code=…; otherwise a signed-in password change.
  ...
  async function doSubmit() {
    ...
    try {
      if (code) await finishPasswordReset(code, pw)
      else await updatePassword(pw)
  ...
  return (
    <>
    {busy && <PageLoader label="Updating password" />}
    <Artboard mobile={mobile}>
      ...
      <form onSubmit={submit}>
        ...
        {msg && <p role={msg.ok ? 'status' : 'alert'} style={at(92, 690, 300, undefined, pop(500, 12, 18, msg.ok ? '#166448' : '#b65a50', { textAlign: 'center' }))}>{msg.text}{msg.ok && <> <Link href="/login" style={{ color: '#18684b' }}>Log in</Link></>}</p>}
      </form>
...
export default function ChangePasswordPage() {
  return <Suspense><ChangePassword /></Suspense>
}
