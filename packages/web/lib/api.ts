...
export async function completeTask(token: string | undefined, id: string, point: number): Promise<Task> {
  if (isMockMode) {
    const task = taskFixtures.find((item) => item.id === id) || taskFixtures[0]
    return { ...task, status: 'Completed', point }
  }
  return request<Task>('/v1/org/complete-task', { method: 'PUT', body: { taskID: id, point }, token })
}

export async function addComment(token: string | undefined, comment: Comment): Promise<Comment> {
  if (isMockMode) {
